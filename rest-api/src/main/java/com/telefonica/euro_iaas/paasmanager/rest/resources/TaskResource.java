...
public interface TaskResource {
     ...
     * @param states the list containing the task states
     * @param resource the resource url the task is related to
     * @param owner the task's owner url
     * @param fromDate the initial date where the task was created (included)
     * @param toDate the final date where the task was created (included)
     * @return the tasks that match with the criteria.
     */
    @GET
    @Path("/")
    @Produces( { MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    List<Task> findAll(@QueryParam("page") Integer page,
