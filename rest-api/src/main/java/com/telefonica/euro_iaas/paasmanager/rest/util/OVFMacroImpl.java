...
public class OVFMacroImpl implements OVFMacro {
	...
	// login(vm)
	// password(login)
	// el patron será "name(source)" Name es el nombre de la propiedad y
	 ...
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.util.OVFMacro#convertMacros(com.
	 * telefonica.euro_iaas.paasmanager.util.EnvironmentInstance)
	 */
	public Environment resolveMacros(Environment environment)
		...
		for (int i = 0; i < macros.size(); i++) {
			...
			String macroReference = (String) macroParts[1].substring(0,
					macroParts[1].length() - 1);

			if (macroProperty.equals(MACRO_NAME_IP)) {
				values.add(macros.get(i));
			} else {
				values.add(getProductAttributeFromEnvironment(environment,
						macroProperty, macroReference));
			}
	 ...
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.util.OVFMacro#getAllMacros(java.
	 * lang.String)
	 */
	/*
	 * private HashMap<String, String> getMacroKeyValue(String xml) {
	 * log.info("OVFMacroImpl.getAllMacros()"); HashMap<String, String> macros =
	 * new HashMap<String, String>(); String[] parts = xml.split("\"");
	 * List<String> macroList = new ArrayList<String>(); for (int i= 0; i <
	 * parts.length; i++){ if (parts[i].startsWith("@"))
	 * macroList.add(parts[i]); }
	 * 
	 * for (int j=0; j < macroList.size(); j++) { String macroProperty =
	 * macroList.get(j).split("(")[0].substring(1); String macroReference =
	 * macroList.get(j).split("(")[1].substring(0,
	 * macroList.get(j).split("(")[1].length() -1); macros.put(macroProperty,
	 * macroReference); } return macros; }
	 */
	...
	/*
	 * private List<String> getAllMacros(String xml) {
	 * log.info("OVFMacroImpl.getAllMacros()"); String[] parts =
	 * xml.split("\""); List<String> macroList = new ArrayList<String>(); for
	 * (int i= 0; i < parts.length; i++){ if (parts[i].startsWith("@"))
	 * macroList.add(parts[i]); } return macroList; }
	 */
	 ...
	 * @return
	 */
	private String getIPFromEnvironmentInstance(
			EnvironmentInstance envInstance, String vmname, String network)
			throws InvalidEnvironmentRequestException {
		...
		if (ip == null) {
			String errorMessage = "The VM " + vmname + " is not present in "
					+ " the environmentInstace: " + envInstance.getName();
			log.error(errorMessage);
			throw new InvalidEnvironmentRequestException(errorMessage);
		}
	...
	private String getProductAttributeFromEnvironment(Environment environment,
			String macroAttribute, String macroProductName)
			throws InvalidEnvironmentRequestException {
		...
		if (macroValue == null) {
			String errorMessage = " Attribute " + macroAttribute
					+ " is not present in ProductReleases of Environment "
	 ...
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.util.OVFMacro#setMacroValue(java
