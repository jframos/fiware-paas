 ...
 * 
 */
public class EnvironmentResourceValidatorImpl implements
		EnvironmentResourceValidator {

	private static Logger log = Logger
			.getLogger(EnvironmentResourceValidatorImpl.class);
	...
	public void validateCreate(EnvironmentDto environmentDto, String vdc,
		...
		try {
			...
			log.error("The enviornment " + environmentDto.getName()
					+ " already exists");
			throw new AlreadyExistEntityException("The enviornment "
					+ environmentDto.getName() + " already exists");
			...
			if (environmentDto.getName() == null) {
				...
				throw new InvalidEnvironmentRequestException(
						"EnvironamentName " + "from EnviromentDto is null");
			}
		...
		if (environmentDto.getTierDtos() != null) {
			...
			for (TierDto tierDto : environmentDto.getTierDtos()) {
				...
				try {
					...
					throw new InvalidEnvironmentRequestException("The tier "
							+ tierDto.getName() + " already exist in the vdc "
							+ vdc, e);
				}
	...
	public void validateUpdate(String environmentName, String vdc,
			SystemPropertiesProvider systemPropertiesProvider)
			throws InvalidEnvironmentRequestException,
			AlreadyExistEntityException, InvalidEntityException {
