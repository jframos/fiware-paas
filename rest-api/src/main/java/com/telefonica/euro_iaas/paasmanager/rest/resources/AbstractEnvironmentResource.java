 ...
 */
public interface AbstractEnvironmentResource {

    /**
     * Add the selected Environment in to the SDC catalog. If the Environment already exists, then add the new Release.
     */
    ...
    @Path("/{envName}")
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    EnvironmentDto load(@PathParam("org") String org, @PathParam("envName") String name) throws APIException;
    ...
    @Path("/{envName}")
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    void delete(@PathParam("org") String org, @PathParam("envName") String envName) throws APIException;
