...
public class TierResourceValidatorImpl implements TierResourceValidator {
	...
	private static Logger log = Logger
			.getLogger(TierResourceValidatorImpl.class);
	...
	public void validateCreate(TierDto tierDto, String vdc, String environmentName,
		...
		if (tierDto == null) {
			...
			throw new InvalidEntityException(tierDto, new Exception(
					"Tier Name " + " is null"));
		}
			...
			try {
				...
				throw new AlreadyExistEntityException("The tier "
						+ tierDto.getName() + " already exists in vdc " + vdc + " and environmentName " + environmentName);
	...
	private void validateCreateTier (TierDto tierDto, SystemPropertiesProvider systemPropertiesProvider) throws InvalidEntityException
	{
		...
		if (tierDto.getName() == null) {
			log.error("Tier name is Null");
			throw new InvalidEntityException(tierDto, new Exception(
					"Tier Name " + "from tierDto is null"));
		}
		if (tierDto.getMaximumNumberInstances() == null
				|| tierDto.getMinimumNumberInstances() == null
				|| tierDto.getInitialNumberInstances() == null) {
			log.error("Number initial, maximun o minimul from tierDto " + tierDto.getName() + " is null");
			throw new InvalidEntityException(tierDto, new Exception(
					"Number initial, maximun o minimul  "
							+ "from tierDto is null"));
		}
		
		if (!(tierDto.getMinimumNumberInstances()<= tierDto.getInitialNumberInstances() &&
				tierDto.getMaximumNumberInstances() >=tierDto.getInitialNumberInstances()))
		{
			String men = "Error in the Number initial " + tierDto.getInitialNumberInstances() + " with number min " + tierDto.getMinimumNumberInstances() +
			" and number max " + tierDto.getMaximumNumberInstances();
			log.error(men);
		...
		if ("FIWARE".equals(system)) {
			if (tierDto.getImage() == null) {
				throw new InvalidEntityException(tierDto, new Exception(
						"Tier Image " + "from tierDto is null"));
			}
			if (tierDto.getFlavour() == null) {
				throw new InvalidEntityException(tierDto, new Exception(
						"Tier Flavour " + "from tierDto is null"));
			}
	...
	public void validateUpdate(TierDto tierDto, String vdc, String environmentName,
		...
		try {
			...
			log.error("The tier " + tierDto.getName() + " does not  exists vdc " + vdc + " environmentName " +  environmentName );
	...
	public void validateDelete(String vdc, String environmentName,
			SystemPropertiesProvider systemPropertiesProvider)
			throws InvalidEntityException, EntityNotFoundException {
	...
	private void validateTierInEnvInstance (String environmentName, String vdc) throws InvalidEnvironmentRequestException {
		Environment environment = null;
		
		try {
			environment = environmentManager.load(environmentName,vdc);
		} catch (EntityNotFoundException e) {
			log.error("The enviornment " + environmentName
					+ " does not exist");
			throw new InvalidEnvironmentRequestException ("The enviornment " + environmentName
					+ " does not exist");
		}
		...
		if (envInstances != null && envInstances.size() != 0) {
			...
			"The enviornmetn is being used by an env instance");
		}
