...
=======
     * @see com.telefonica.euro_iaas.paasmanager.rest.util.OVFGeneration#createOvf
...
<<<<<<< HEAD
        ...
        for (int i = 0; i < tierDto.getProductReleaseDtos().size(); i++) {
            ProductReleaseDto productReleaseDto = tierDto
            .getProductReleaseDtos().get(i);
            // ProductName
...
=======
    private String setInfoTier(TierDto tierDto) throws IOException {
        ...
        if (tierDto.getKeypair() == null && tierDto.getFloatingip() == null && tierDto.getSecurity_group() == null) {
            return "";
        }
        ...
        infoTemplate = replace(infoTemplate, "\\$\\{security_group\\}", tierDto.getSecurity_group());
        return infoTemplate;
