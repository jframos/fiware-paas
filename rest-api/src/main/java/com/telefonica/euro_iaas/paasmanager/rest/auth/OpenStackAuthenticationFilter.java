...
public class OpenStackAuthenticationFilter extends GenericFilterBean {
     ...
     *
     * @param pAuthenticationManager the bean to submit authentication requests
     * to {@code AuthenticationManager} and which will ignore failed
     * authentication attempts, allowing the request to proceed down the filter
     * chain.
     */
    public OpenStackAuthenticationFilter(
     ...
     * @param pAuthenticationManager the bean to submit authentication requests
     * to
     * @param pAuthenticationEntryPoint will be invoked when authentication
     * fails. Typically an instance of {@link BasicAuthenticationEntryPoint}.
    ...
    public final void doFilter(final ServletRequest req,
        ...
        if (pathInfo.equals("/") || pathInfo.equals("/extensions")) {
            ...
            try {
                ...
                if (debug) {
                    logger
                      .debug("OpenStack Authentication Authorization header "
                            + "found for user '" + token + "'");
                }
        		...
        		System.out.println("Tenant: "+ user.getTenantId());
        		System.out.println("TenantName - Org: "+ user.getTenantName());
                ...
                if (debug) {
                    logger
                       .debug("Authentication request for failed: " + failed);
                }
            ...
            response.addHeader("Www-Authenticate", "Keystone uri='"
                    + keystoneURL + "'");
        }
        chain.doFilter(request, response);
     ...
     */
    public final AuthenticationDetailsSource<HttpServletRequest, ?> 
     ...
     *
     * @param pIgnoreFailure the new ignore failure
     */
     ...
     *
     * @param pAuthenticationEntryPoint the new authentication manager
     */
    public final void setAuthenticationManager(
            final AuthenticationManager pAuthenticationEntryPoint) {
     ...
     *
     * @param pAuthenticationEntryPoint the new authentication entry point
     */
    public final void setAuthenticationEntryPoint(
            final AuthenticationEntryPoint pAuthenticationEntryPoint) {
        this.authenticationEntryPoint = pAuthenticationEntryPoint;
    }
     ...
     *
     * @param pRememberMeServices the new remember me services
     */
    public final void setRememberMeServices(
            final RememberMeServices pRememberMeServices) {
     ...
     *
     *
     * final String username = OPENSTACK_IDENTIFIER;
     *
     * String token = request.getHeader(OPENSTACK_HEADER_TOKEN);
     *
     * if (token == null) { logger.debug("Failed to obtain an artifact
     * (openstack tocken)"); token = ""; }
     *
     * UsernamePasswordAuthenticationToken authRequest = new
     * UsernamePasswordAuthenticationToken(username, token);
     * authRequest.setDetails
