...
public class OpenStackAuthenticationFilter extends GenericFilterBean {
	 ...
	 *            the bean to submit authentication requests to {@code
	 *            AuthenticationManager} and which will ignore failed
	 ...
	 *            instance of {@link BasicAuthenticationEntryPoint}. {@code
	 *            AuthenticationManager} and use the supplied {@code
	 *            AuthenticationEntryPoint} to handle authentication failures.
	 */
	public OpenStackAuthenticationFilter(
			final AuthenticationManager pAuthenticationManager,
	...
	public final void doFilter(final ServletRequest req,
		...
		if (pathInfo.equals("/") || pathInfo.equals("/extensions")) {
			...
			try {
				...
				PaasManagerUser user = (PaasManagerUser) authResult
						.getPrincipal();
				...
				if (debug) {
					logger
							.debug("Authentication request for failed: "
									+ failed);
				}
