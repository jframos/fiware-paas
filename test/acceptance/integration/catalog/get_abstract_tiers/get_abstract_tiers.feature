# -*- coding: utf-8 -*-
Feature: Get the list of tiers of an abstract environment

    As a fi-ware user
    I want to be able to get the list of tiers of an abstract environment
    so that I know the tiers my abstract environment will have when I create new instances
    ...
    Scenario Outline: Get a list of just one tier with different valid data
        ...
        And a tier has already been added to the abstract environment "<name>" with data:
            | name       |
            | <tiername> |
        When I request the list of tiers of the abstract environment "<name>"
        Then I receive an "OK" response with "1" item in the list
        And there is a tier in the list with data:
            | name       |
            | <tiername> |
