...
public class InfrastructureManagerImplTest {
    ...
    public void setUp() throws Exception {
		...
		org = "ORG";
		number_vms = 2;
		claudiaData = new ClaudiaData(org, vdc);
		...
		vdcResponseTask.setResource("http://10.95.171.89:8080/rest-api-management/",
				"resourceType");
		serviceResponseTask = new Task();
		serviceResponseTask.setState(TaskStates.SUCCESS);
		serviceResponseTask.setResource("http://10.95.171.89:8080/rest-api-management/",
				"resourceType");
		
		vmResponseTask = new Task();
		vmResponseTask.setState(TaskStates.SUCCESS);
		vmResponseTask.setResource("http://10.95.171.89:8080/rest-api-management/",
				"resourceType");
			
		propertiesProvider = mock(SystemPropertiesProvider.class);
		when(propertiesProvider.getProperty(NEOCLAUDIA_SERVICE)).thenReturn("paasmanagerService");
		when(propertiesProvider.getProperty(NEOCLAUDIA_VDC_CPU)).thenReturn("12");
		...
		when(propertiesProvider.getProperty(VM_NAME_PREFIX)).thenReturn("paasManagerVM");
		
		
		claudiaClient = mock(ClaudiaClient.class);
		//when(claudiaClient.browseVDC(any(String.class), any(String.class), user))
			//.thenReturn(vdcResponse);
		when(claudiaClient.browseVDC(any(ClaudiaData.class))).thenReturn(vdcResponse);
		when(claudiaClient.deployVDC(any(ClaudiaData.class), 
				any(String.class), any(String.class), any(String.class))).thenReturn("OK");
		when(claudiaClient.browseService(any(ClaudiaData.class))).thenReturn(vdcResponse);
		when(claudiaClient.deployService(any(ClaudiaData.class), any(String.class)))
					.thenReturn("OK");
		when(claudiaClient.deployVM(any(String.class), any(String.class),
				any(String.class), any(String.class), user, any(String.class)))
					.thenReturn("OK");
		
		when(claudiaClient.deployVM(any(ClaudiaData.class),any(String.class)))
			.thenReturn("OK");
		
		when(claudiaClient.obtainIPFromFqn(any(String.class), any(String.class),
				any(String.class),any(String.class), user))
