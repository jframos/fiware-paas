...
public class OpenstackFirewallingClientImpl implements FirewallingClient {
	...
	private static Logger log = Logger
			.getLogger(OpenstackFirewallingClientImpl.class);


	/* (non-Javadoc)
	 * @see com.telefonica.euro_iaas.paasmanager.claudia.FirewallingClient#loadAllSecurityGroups(com.telefonica.euro_iaas.paasmanager.model.ClaudiaData)
	 */
	public List<SecurityGroup> loadAllSecurityGroups(ClaudiaData claudiaData) throws OpenStackException {
		...
		if (response.getStatus() != 200) {
			String message = "Error calling OpenStack to recover all secGroups. " +
	...
	public SecurityGroup loadSecurityGroup(ClaudiaData claudiaData, String securityGroupId)
		...
		String url = systemPropertiesProvider.getProperty("openstack.nova.url")
				...
				+ claudiaData.getVdc() + "/os-security-groups/" + securityGroupId;
		log.debug("actionUri: " + url);
	...
	public String deploySecurityGroup(ClaudiaData claudiaData,
	...
	public String deployRule(ClaudiaData claudiaData, Rule rule)
			throws InfrastructureException {
		return createRule(claudiaData, rule);

	}

	public String createSecurityGroup(ClaudiaData claudiaData,
			SecurityGroup securityGroup) throws InfrastructureException {
...
//                                }
	...
	public String createRule(ClaudiaData claudiaData, Rule rule)
			throws InfrastructureException {
...
//                                }
	...
	public void destroySecurityGroup(ClaudiaData claudiaData,
			SecurityGroup securityGroup) throws InfrastructureException {
		...
		try {
			...
			if (response.getStatus() == 200 || response.getStatus() == 202
				...
				log.error("Error to delete a security group " + securityGroup
						+ " : " + result);
				throw new InfrastructureException(
						"Error to delete a security group " + securityGroup
								+ " : " + result);
			}
	...
	public void destroyRule(ClaudiaData claudiaData, Rule rule)
		...
		try {
			...
			if (response.getStatus() == 200 || response.getStatus() == 202
				...
				log.error("Error to delete a security rule " + rule.getIdRule()
						+ ": " + result);
				throw new InfrastructureException(
						"Error to delete a security rule " + rule.getIdRule()
								+ ": " + result);
			}
