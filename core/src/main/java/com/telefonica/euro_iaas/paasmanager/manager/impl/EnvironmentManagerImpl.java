...
public class EnvironmentManagerImpl implements EnvironmentManager {
	...
	public Environment create(ClaudiaData claudiaData, Environment environment)
		...
		log.debug("Creating environment " + environment.getName()
				+ " with escription " + environment.getDescription() + " org "
				+ environment.getOrg() + " vdc " + environment.getVdc()
				+ " tiers  " + environment.getTiers());
			...
			for (Tier tier : environment.getTiers()) {
				...
				try {
					...
					try {
						...
						try {
							tierDB = tierManager.create(claudiaData, environment.getName(),
									tier);
		...
		/*
		 * try { environmentType = environmentTypeDao.load(environment
		 * .getEnvironmentType().getName()); } catch (EntityNotFoundException e)
		 * { try { environmentType = environmentTypeDao.create(environment
		 * .getEnvironmentType()); } catch (AlreadyExistsEntityException e1) {
		 * String errorMessage = "The EnvironmentType Object " +
		 * environmentType.getName() + "is already in Database"; throw new
		 * InvalidEnvironmentRequestException (errorMessage); } catch
		 * (InvalidEntityException e2) { String errorMessage =
		 * " The EnvironmentType Object " + environmentType.getName() +
		 * "is invaled" + e2.getMessage(); throw new
		...
		try {
			...
			String errorMessage = "The Environment Object "
					+ environment.getName() + " is " + "NOT valid: "
					+ iee.getMessage();
	...
	public void destroy(ClaudiaData data, Environment environment)
	...
	public Environment updateTier(Environment environment, Tier tierold, Tier tiernew)
	throws EntityNotFoundException, InvalidEntityException {
