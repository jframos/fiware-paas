...
public class OpenstackDummyImpl implements ClaudiaClient {
	...
	public String browseService(ClaudiaData claudiaData)
		...
		String payload = null;
		try {
			payload = fileUtils.readFile(systemPropertiesProvider
					.getProperty("neoclaudiaVappVMLocation"));
		} catch (FileUtilsException e) {
			throw new ClaudiaResourceNotFoundException(
					"Error in the Claudia Dummy Utils " + e.getMessage());
		}
		return payload;
	}
	...
	public String browseVMReplica(ClaudiaData claudiaData, String tierName,
		...
		String ip = null;
		try {
			ip = obtainIPFromFqn(claudiaData.getOrg(), claudiaData.getVdc(),
					claudiaData.getService(), tierName, claudiaData.getUser());
			ip = obtainIPFromFqn(claudiaData.getOrg(), claudiaData.getVdc(),
					claudiaData.getService(), tierName, claudiaData.getUser());
		...
		try {
			payload = fileUtils.readFile(
					systemPropertiesProvider
							.getProperty("neoclaudiaVappVMLocation")).replace(
					"{org}", claudiaData.getOrg()).replace("{vdc}",
					claudiaData.getVdc()).replace("{service}",
					claudiaData.getService()).replace("{vm}", tierName)
	...
	public List<String> getIP(ClaudiaData claudiaData, String tierName,
		...
		try {
			String ip = obtainIPFromFqn(claudiaData.getOrg(), claudiaData
					.getVdc(), claudiaData.getService(), tierName, claudiaData
					.getUser());
			ips.add(ip);
		} catch (IPNotRetrievedException e1) {
			// TODO Auto-generated catch block
	...
	public void deployVM(ClaudiaData claudiaData, Tier tier, int replica, VM vm)
		...
		String url = "http://130.206.80.63:8774/v2/" + claudiaData.getVdc()
				+ "/servers";
		...
		try {
			...
			String id = result.substring(result.indexOf("\"id\": \"")
					+ "\"id\": \"".length(), result.indexOf(", \"links\"") - 1);
	...
	private Map<String, String> getHeaders(PaasManagerUser claudiaData) {
		...
		/*
		 * headers.put("X-Auth-Token", "bc3fd7adc68643d28a386a0fe5a48f7c");
		 * headers.put("X-Auth-Project-Id", "6571e3422ad84f7d828ce2f30373b3d4");
		 */
	...
	public String getVApp(String org, String vdc, String service,
		...
		try {
			payload = fileUtils.readFile("VappTemplate.xml",
					"./src/main/resources").replace("{org}", org).replace(
					"{vdc}", vdc).replace("{service}", service).replace("{vm}",
					vmName).replace("{replica}", "1").replace("{IP}", ip);
	...
	public String obtainIPFromFqn(String org, String vdc, String service,
		...
		String url = "http://130.206.80.63:8774/v2/" + vdc + "/servers/"
				+ this.auxIdVM;
		System.out.println("actionUri: " + url);
		try {
			...
			String ip = result.substring(result.indexOf("\"addr\": \"")
					+ "\"addr\": \"".length(), result.indexOf("\"}]}"));
