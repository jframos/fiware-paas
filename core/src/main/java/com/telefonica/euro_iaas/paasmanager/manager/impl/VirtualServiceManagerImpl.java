...
public class VirtualServiceManagerImpl implements VirtualServiceManager {
	...
	private static Logger log = Logger
			.getLogger(VirtualServiceManagerImpl.class);

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.manager.VirtualServiceManager#create
	 * (java.lang.String, java.lang.String)
	 */
	public void create(String virtualServiceName, String payload)
		...
		try {
			...
			String msg = "Error getting AC ProductSections from ovf. "
					+ "Desc: " + e.getMessage();
			log.info(msg);
			throw new VirtualServiceInstallationException(msg);
		}
		...
		recVirtualServiceService = client.getRECVirtualServiceService(baseUrl,
				mediaType);
		for (int i = 0; i < acProductSections.size(); i++) {
			...
			try {
				...
				String msg = "Error getting picId from ProductSection of type AC. "
						+ "Desc: " + e.getMessage();
				log.info(msg);
				throw new VirtualServiceInstallationException(msg);
			}
			recVirtualServiceService.createVirtualService(appId, vmId, picId,
					acId, acProductSections.get(i));
		}
	}
