...
public class MonitoringClientCollectdImpl implements MonitoringClient {
    ...
    private static Logger log = Logger.getLogger(MonitoringClientCollectdImpl.class);
    ...
    public String getMonitoringStatus(String fqn) {
        ...
        String uri = MessageFormat.format(systemPropertiesProvider.getProperty(COLLECTOR_BASEURL),
                systemPropertiesProvider.getProperty(COLLECTOR_IP),
                systemPropertiesProvider.getProperty(COLLECTOR_PORT), md5);
        ...
        try {
            ...
            if (vmResponse == 200) {
                ...
                String errorMessage = "Exception when trying to start monitoring " + "System from VM with the fqn: "
                        + fqn + ". Invalid Monitoring" + "Response";
                log.error(errorMessage);
                // throw new MonitoringException(errorMessage);
            }
        } catch (MonitoringGetException e) {
            String errorMessage = "Exception when starting monitoring on " + "machine" + fqn + " GET on " + uri
    ...
    public void stopMonitoring(String fqn) {
        ...
        try {
            ...
            String errorMessage = "Exception when deleting monitoring on " + "machine" + fqn + " DELETE on " + uri
                    + "FAILED. Invalid " + "Monitoring Request";
            log.error(errorMessage);
            // throw new MonitoringException (errorMessage);
        } catch (MonitoringGetException e) {
            String errorMessage = "Exception when getting monitoring on " + "machine" + fqn + " GET on " + uri
                    + "FAILED. Invalid " + "Monitoring Request";
            log.error(errorMessage);
            // throw new MonitoringException (errorMessage);
        }
            ...
            try {
                ...
                if ((vmResponse >= 300) || (vmResponse < 200)) {
                    String errorMessage = "Exception when trying to start monitoring "
                            + "System from VM with the fqn: " + fqn + ". Invalid Monitoring" + "Response";
                    log.error(errorMessage);
                    // throw new MonitoringException(errorMessage);
                }
            } catch (MonitoringPutException e) {
                String errorMessage = "Exception when  starting monitoring on " + "machine" + fqn + " PUT on " + uri
                        + " with payload " + fqn + "FAILED. Invalid Monitoring Request";
                log.error(errorMessage);
                // throw new MonitoringException(errorMessage);
            }
        } else {
            log.info("The fqn: " + fqn + "has been registered in " + "the monitoring systems before,");
        }
    }

    private ClientResponse putMonitoringResource(String url, String fqn) throws MonitoringPutException {
    ...
    private ClientResponse getResourceMonitoring(String url) throws MonitoringGetException {
        Client client = new Client();
        log.info("url: " + url);
        ClientResponse response = null;
        try {
            WebResource wr = client.resource(url);
            response = wr.accept("text/plain").type("text/plain").get(ClientResponse.class);
        } catch (Exception e) {
            String errorMessage = "Error performing get on the resource monitoring: " + url;
            log.error(errorMessage);
    ...
    private ClientResponse deleteResourceMonitoring(String url) throws MonitoringDeleteException {
        Client client = new Client();
        log.info("url: " + url);
        ClientResponse response = null;
        try {
            WebResource wr = client.resource(url);
            response = wr.accept("text/plain").type("text/plain").delete(ClientResponse.class);
        } catch (Exception e) {
            String errorMessage = "Error performing delete on the resource monitoring: " + url;
            log.error(errorMessage);
    ...
    private boolean isCreatedBefore(String fqn) {
        ...
        try {
            ...
            Connection conexion = DriverManager.getConnection(systemPropertiesProvider.getProperty(COLLECTOR_MYSQL),
                    "root", null);
            conexion.setAutoCommit(false);
            Statement s = conexion.createStatement();
            ResultSet rs = s.executeQuery("select count(*) from nodedirectory " + "where fqn like '" + fqn + "'");
            while (rs.next()) {
            ...
            log.error("Have a problem conection with the BD of the minitoring " + "sistem. Describe: " + e);
            count = 1;
        }
