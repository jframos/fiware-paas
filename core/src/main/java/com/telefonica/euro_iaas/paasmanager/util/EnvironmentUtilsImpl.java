...
public class EnvironmentUtilsImpl implements EnvironmentUtils {
	 ...
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.util.EnvironmentUtils#resolveMacros
	...
	public Environment resolveMacros(Environment environment, List<VM> vms) {
		...
		for (int i = 0; i < tiers.size(); i++) {
			List<ProductRelease> productReleases = tiers.get(i)
					.getProductReleases();
			for (int j = 0; j < productReleases.size(); j++) {
				if (productReleases.get(j).getAttributes() != null) {
					List<Attribute> attributes = productReleases.get(j)
							.getAttributes();
						...
						if (attributes.get(k).getValue().contains(IP_MACRO_ID)) {
							...
							for (int l = 0; l < vms.size(); l++) {
								if (vms.get(l).getFqn().contains(vm))
									attributes.get(k).setValue(
											macroLine.replace(ipmacro, vms.get(
													l).getIp()));
							}
						}
					}
				...
				if (productReleases.get(j).getAttributes() != null) {
					...
					for (int k = 0; k < attributes.size(); k++) {
						...
						if (attributes.get(k).getValue().contains(IP_MACRO_ID)) {
							...
							attributes.get(k).setValue(
									macroLine.replace(ipmacro, vm.getIp()));
						}
					}
				}
			}
	...
	private VM getVmWithName(EnvironmentInstance environmetnInstance,
			String nameVM) {
