...
public class ProductReleaseSdcDaoImpl implements ProductReleaseSdcDao {
	...
	private String findAllProductReleases() throws SdcException {
		...
		if (response.getStatus() != 200) {
			String message = "Error calling SDC to recover all product Releases. " +
	...
	private String loadByName(String product, String version) 
			throws EntityNotFoundException, SdcException {
		String url = systemPropertiesProvider.getProperty(
				SystemPropertiesProvider.SDC_SERVER_URL)+ "/catalog/product/" +
		...
		if (response.getStatus() == 404) {
			String message = "The Product Release " + product + "-" + version 
					+ " is not present in SDC DataBase ";
			log.error(message);
