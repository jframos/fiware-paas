...
public class EnvironmentInstanceDaoJpaImpl extends
		AbstractBaseDao<EnvironmentInstance, String> implements
	...
	public EnvironmentInstance loadForDelete(String name)
	 ...
	 * findByCriteria(com.telefonica.euro_iaas.paasmanager.model.searchcriteria.
...
/*	public List<EnvironmentInstance> findByCriteria(
		...
		List<EnvironmentInstance> environments = setOptionalPagination(criteria,
				baseCriteria).list();
		...
		if (!StringUtils.isEmpty(criteria.getVdc()) && !StringUtils.isEmpty(criteria.getEnvironmentName())) {
			environments = filterByEnvironment(environments, 
		...
		else if (!StringUtils.isEmpty(criteria.getVdc())) {
		...
		/*
		 * if (criteria.getApplicatonRelease() != null) { applicationInstances =
		 * filterByApplicationRelease( applicationInstances,
	...
	public List<EnvironmentInstance> findByCriteria(
		...
		baseCriteria.add(Restrictions.eq(InstallableInstance.VDC_FIELD,
				criteria.getVdc()));
		}
		if (!StringUtils.isEmpty(criteria.getEnvironmentName())) {
			baseCriteria.add(Restrictions.eq("blueprintName",
	 ...
	 * .String)
	 */
	private EnvironmentInstance findByEnvironmentInstanceName(
			String envInstanceName) throws EntityNotFoundException {

		Query query = entityManager
				.createQuery("select p from EnvironmentInstance"
		...
		try {
			...
			String message = " No EnvironmentInstance found in the database with tiers"
					+ "with blueprintName: " + envInstanceName;
			System.out.println(message);
			throw new EntityNotFoundException(EnvironmentInstance.class,
					"blueprintName", envInstanceName);
		}
		return environmentInstance;
	}
	
	private EnvironmentInstance findByEnvironmentInstanceNameNoTierInstances(
			String envInstanceName) throws EntityNotFoundException {
		...
		try {
			...
			String message = " No EnvironmentInstance found in the database no tiers "
					+ "with blueprintName: " + envInstanceName;
			System.out.println(message);
			throw new EntityNotFoundException(EnvironmentInstance.class,
					"blueprintName", envInstanceName);
