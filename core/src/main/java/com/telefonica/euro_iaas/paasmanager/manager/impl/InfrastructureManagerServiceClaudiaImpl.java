...
public class InfrastructureManagerServiceClaudiaImpl implements
	...
	private static Logger log = Logger
			.getLogger(InfrastructureManagerServiceClaudiaImpl.class);
	...
	public List<VM> createEnvironment(EnvironmentInstance envInstance,
			Tier tier, String ovf, ClaudiaData claudiaData)
	...
	public EnvironmentInstance createInfrasctuctureEnvironmentInstance(
			...
			InvalidVappException, InvalidOVFException {
		...
		deployService(claudiaData, environmentInstance.getEnvironment()
				.getOvf());
		...
		try {
			tierInstances = fromVappToListTierInstance(vappService,
					environmentInstance.getEnvironment(), claudiaData);
		} catch (InvalidVappException e) {
			throw new InvalidVappException("Error parsing the Vapp "
					+ e.getMessage());
		} catch (InvalidOVFException e1) {
			throw new InvalidVappException(
					"Error obtaining the OVF for the Tier " + e1.getMessage());
		}
	...
	public List<TierInstance> fromVappToListTierInstance(String vappService,
			Environment environment, ClaudiaData claudiaData)
			throws InvalidVappException, InvalidOVFException {
		...
		for (Tier tier: environment.getTiers()) {
			...
			try {
				tierInstance = createTierInstanceFromVapp(tier, environment.getName(),
						claudiaData, xVapp, ovf);

				tierInstances.add(tierInstance);
			} catch (InvalidVappException e) {
				throw new InvalidVappException("Error to parse the VApp"
						+ e.getMessage());
			}
	...
	public TierInstance createTierInstanceFromVapp(Tier tier,
			String environmentName, ClaudiaData claudiaData, String vapp,
			String vmOVF) throws InvalidVappException {
		log.info("createTierInstanceFromVapp " + tier.getName() + "  env " + environmentName+ " size vapp " +  vapp.length());
		...
		try {
			...
			throw new InvalidVappException(
					"Error to obtain the VM from the VApp " + e.getMessage());
		}
		...
		tierInstance.setName(environmentName + "-" + tier.getName() + "-"
				+ numReplica);
	 ...
	 * @throws InfrastructureException
	 */
	public void deployVDC(ClaudiaData claudiaData)
			throws InfrastructureException {
		// VDC
		log.warn("Deploy VDC");
		try {
	 ...
	 * @throws InfrastructureException
	 */
	public void deployService(ClaudiaData claudiaData, String ovf)
			throws InfrastructureException {
	...
	public String getProcessService(ClaudiaData claudiaData)
			throws InfrastructureException {
		...
		try {
			...
			throw new InfrastructureException(
					"It is not possible to build the url for get service");
		}

		try {
			serviceResponse = claudiaUtil.getClaudiaResource(claudiaData
					.getUser(), serviceUrl, MediaType.WILDCARD);
		} catch (ClaudiaRetrieveInfoException e1) {

			throw new InfrastructureException(
					"It is not possible to get the url for get service "
							+ serviceUrl);
		...
		while (cont <= 30) {
			try {
				...
				try {
					serviceResponse = claudiaUtil.getClaudiaResource(
							claudiaData.getUser(), serviceUrl,
							MediaType.WILDCARD);
				} catch (ClaudiaRetrieveInfoException e1) {
		...
		if (cont >= 30) {
			log
					.error("Error to deploy the service. It is not possible to get its information");
			throw new InfrastructureException(
					"It is not possible to get the information about the service. It is not deployed "
							+ serviceUrl);
		}
		return null;
	}

	public void getProcessVapp(ClaudiaData claudiaData, String vappService)
