...
public class OpenStackSyncImpl extends Thread implements OpenStackSync  {
	 ...
	 */
	public OpenStackSyncImpl (Connection connection, boolean onecycle,
			TierDao tierDao, TierInstanceDao tierInstanceDao, FirewallingClient firewallingClient,
			ClaudiaClient claudiaClient, SystemPropertiesProvider systemPropertiesProvider,
	...
	public void syncronize( Connection connection, boolean onecycle) throws OpenStackSynchronizationException {
		...
		while (run) {
			try {
				...
				Thread.sleep(Integer.parseInt(systemPropertiesProvider.getProperty(
					SystemPropertiesProvider.OPENSTACK_SYNCHRONIZATION_POLLING_PERIOD)));
	...
	private List<PaasManagerUser> getUsersFromKeystoneDataBase (Connection connection) {
		...
		for (int i=0; i < keystoneUsers.size(); i ++) {
			try {
				...
				log.debug ("User " + keystoneUsers.get(i).getName()+ " " + token.getTenantId() 
						+ " token " +  token.getId());
						
				PaasManagerUser paasManagerUser = new PaasManagerUser(
						keystoneUsers.get(i).getName(), token.getId(), gas);
				...
				String message = "Error to obtain the token for the user  " + keystoneUsers.get(i).getName() + " : " + e.getMessage();
				log.info(message);
			}	
		...
		for (int i=0; i < securityGroups.size(); i++ ){
			...
			if (!included){
				...
				try {
					...
					log.info("SecurityGroup " + securityGroup.getIdSecurityGroup() + " deployed and syncrhonized with Database");
					securityGroupDao.updateSecurityGroupId(securityGroupId, securityGroup);
				} catch (InfrastructureException e1) {
					String message = "Error re-creating the securityGroup : " 
						+ securityGroup.getIdSecurityGroup();
					log.error(message);
	...
	/*private SecurityGroup findRightSecurityGroup(SecurityGroup securityGroup, 
		List<SecurityGroup> securityGroupsOpenStack) {
		
		for (int i=0; i < securityGroupsOpenStack.size(); i ++) {
			if (securityGroup.getName().equals(securityGroupsOpenStack.get(i).getName())){
				...
				securityGroup.setRules(securityGroupsOpenStack.get(i).getRules());	
