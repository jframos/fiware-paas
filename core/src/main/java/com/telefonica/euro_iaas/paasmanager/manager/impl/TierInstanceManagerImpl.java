...
<<<<<<< HEAD
    ...
    throws InvalidEntityException {
        log
        .debug("Inserting in database tierInstance"
        ...
        try {
            ...
            throw new InvalidEntityException(
                    "Error to create the Tier Instance "
                    + tierInstance.getName() + ". It already exists. ");
        ...
        try {
            ...
            log.error("Error to load the Tier "
                    + tierInstance.getTier().getName() + " : " + e.getMessage());
            throw new InvalidEntityException("Error to load the Tier "
                    + tierInstance.getTier().getName() + " : " + e.getMessage());
        }
        tierInstanceDB.setTier(tierDB);

        if (tierInstance.getProductInstances() != null) {
            for (ProductInstance productInstance : tierInstance
                    .getProductInstances()) {
                ProductInstance productInstanceDB = null;
                try {
                    productInstanceDB = productInstanceManager
                    .load(productInstance.getName());
                } catch (EntityNotFoundException e) {
                    try {
                        productInstanceDB = productInstanceManager
                        .create(productInstance);
                    } catch (InvalidEntityException e1) {
                        throw new InvalidEntityException("Error to create the "
                                + "product instance "
        ...
        try {
            ...
            throw new InvalidEntityException(
                    "Error to create the tier instance "
                    + tierInstanceDB.getName() + " : " + e.getMessage());
        }
    ...
    ProductInstallatorException {
        ...
        + claudiaData.getService() + ".vees." + tierInstance.getTier().getName()
        + ".replicas." + replicaNumber;
        String hostname = claudiaData.getService() + "-" + tierInstance.getTier().getName() + "-"
        + replicaNumber;
=======
...
<<<<<<< HEAD
    ...
    private List<Attribute> getPICAttributesOVF(String vmOVF) {
        ...
        if (vmOVF.contains("ovfenvelope:value=\"PIC\"")) {
            first = vmOVF.split("ovfenvelope:value=\"PIC\"", 2)[1];
            property = first.split(
                    "Product Specific Attributes</ovfenvelope:Category>", 2)[1];
            while (property.contains("<ovfenvelope:Property")) {
                lValue = property.split("ovfenvelope:value=\"", 2)[1].split(
                        "\"", 2);
                value = lValue[0];
                lKey = lValue[1].split("key=\"", 2)[1].split("\"", 2);
                key = lKey[0];
...
<<<<<<< HEAD
        ...
        if (environment.getTiers() != null)
        {
            for (Tier tier: environment.getTiers())
            {
                ...
                /*try {
					tier = tierManager.load(tier.getName(), environment.getVdc());
				} catch (EntityNotFoundException e) {
					throw new InvalidEntityException (e);
				}*/
                if (tier.getProductReleases() != null)
...
=======
        ...
        if (environment.getTiers() != null) {
            for (Tier tier : environment.getTiers()) {
                ...
                /*
                 * try { tier = tierManager.load(tier.getName(), environment.getVdc()); } catch (EntityNotFoundException
                 * e) { throw new InvalidEntityException (e); }
                 */
                if (tier.getProductReleases() != null) {
                    for (ProductRelease productRelease : tier.getProductReleases()) {
                        if (productRelease.getProduct().equals(productName)) {
...
=======
    ...
    /*
     * public Attribute getAttribute(List<Attribute> attributes, String key) { if (attributes == null) return null; for
     * (Attribute attribute : attributes) { if (attribute.getKey().equals(key)) return attribute; } return null; }
     */
...
<<<<<<< HEAD
    public void reconfigure (ClaudiaData claudiaData, EnvironmentInstance environmentInstance, TierInstance tierInstance) throws ProductReconfigurationException
    {
        log.info("Reconfiguring the balancer for the tier Instance " + tierInstance.getName());
        ...
        log.debug ("Tier instance to reconfigure since it is a balancer " + tierInstanceToConfigured.getName() + " with tier " +
                tierInstanceToConfigured.getTier().getName());
        try {
            update(claudiaData, tierInstanceToConfigured,environmentInstance);
        } catch (Exception e) {
            ...
            log.error("Error to reconfigure the tier " + tierInstanceToConfigured.getName() +" from tier "+
                    tierInstanceToConfigured.getTier().getName () + " " + e.getMessage());
            throw new ProductReconfigurationException ("Error to reconfigure the tier " + tierInstanceToConfigured.getName() +" from tier "+
                    tierInstanceToConfigured.getTier().getName (),e);
        }
        // TODO Auto-generated catch block
        ...
        try {
            ...
            if (replicaNumberAnt > 0) {
                ...
                tierInstance.setProductInstances(tierInstanceAnt
                        .getProductInstances());
            }
        } catch (IllegalArgumentException e) {
            log
            .error("Cannot parse the number of replica in the reconfigutation option: "
                    + e);
        }
=======
    /**
...
<<<<<<< HEAD
        List<ProductInstance> productInstances = tierInstance
        .getProductInstances();
=======
...
<<<<<<< HEAD
    ...
    ProductReconfigurationException {
        ...
        Tier tier = null;
        try {
            tier = tierManager.load(tierInstance.getTier().getName(), claudiaData.getVdc(), envInstance.getEnvironment().getName());
        } catch (EntityNotFoundException e) {
            log.error("No entity found " + tierInstance.getTier().getName() + " wit vdc " + claudiaData.getVdc() +
                    " and entiroment " + envInstance.getEnvironment().getName());
            throw new InvalidEntityException (e);
        ...
        if ((tier.getProductReleases() != null)
                && (tier.getProductReleases().size() != 0)) {

            for (ProductInstance productInstance : tierInstance
                    .getProductInstances()) {

                log.debug("Configure productInstance " + productInstance.getProductRelease().getProduct());
                List<Attribute> atributes = getPICAttributesOVF(tierInstance
                        .getOvf());
                productInstanceManager.configure(claudiaData, productInstance, atributes);
            }
        }
