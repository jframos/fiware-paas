...
public class VirtualServiceAsyncManagerImpl implements VirtualServiceAsyncManager{
	...
	/* (non-Javadoc)
	 * @see com.telefonica.euro_iaas.paasmanager.manager.async.VirtualServiceAsyncManager#create(java.lang.String, com.telefonica.euro_iaas.paasmanager.model.Task, java.lang.String)
	 */
	@Async
	public void create(String virtualServiceName, String virtualService, Task task, String callback) {
		...
		try {	
			...
			log.info("The VirtualService "
	                + virtualServiceName+ " has been CORRECTLY installed");
			...
			String errorMsg = "Unexpected error installing virtualService : " 
					+ virtualServiceName + ". Description:" + 
					e.getMessage();
			updateErrorTaskOnInstall(virtualServiceName, task, errorMsg, e);
		}
		finally {
    ...
    private void updateSuccessTask(Task task, String virtualServiceName) 
        ...
        String piResource = MessageFormat.format(
                propertiesProvider.getProperty(VIRTUAL_SERVICE_BASE_URL),
                virtualServiceName); // the virtualServiceName
     ...
     */
    private void updateErrorTaskOnInstall(String virtualServiceName, 
