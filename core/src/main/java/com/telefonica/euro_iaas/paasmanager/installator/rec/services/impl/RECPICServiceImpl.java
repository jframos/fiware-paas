 ...
 */
public class RECPICServiceImpl extends AbstractBaseService implements RECPICService {
    ...
    public void configurePIC(String vapp, String appId, String picId, String vmName)
        ...
        vapp = vapp.replace("<ovfenvelope:ProductSection>",
                "<ovfenvelope:ProductSection xmlns:ovfenvelope=\"http://schemas.dmtf"
                        + ".org/ovf/envelope/1\" ovfenvelope:class=\"4caast.vm" + ".application\">");

        DomRepresentation data = null;
        Response putResponse = null;

        Reference urlPut = new Reference(getBaseHost()
                + MessageFormat.format(ClientConstants.BASE_PIC_PATH, appId, vmName) + "/" + picId);
        ...
        if (getResponse.getStatus().equals(Status.SUCCESS_OK)) {
            ...
            throw new ProductReconfigurationException("The PIC is not" + "created before");
        }

    }

    public void createPIC(String vapp, String appId, String picId, String vmName) throws ProductInstallatorException {
        ...
        Reference urlPost = new Reference(getBaseHost()
                + MessageFormat.format(ClientConstants.BASE_PIC_PATH, appId, vmName));
