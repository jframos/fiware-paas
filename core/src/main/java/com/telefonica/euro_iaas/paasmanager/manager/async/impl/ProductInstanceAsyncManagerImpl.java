...
public class ProductInstanceAsyncManagerImpl implements
	...
	public void install(TierInstance tierInstance, ClaudiaData claudiaData, String envName,
		...
		try {
			ProductInstance productInstance = productInstanceManager.install(
					tierInstance, claudiaData, envName, productRelease, attributes);
			...
			ProductInstance instance = getInstalledProduct(productRelease,
					tierInstance.getVM());
			if (instance != null) {
	...
	public void uninstall(ProductInstance productInstance, Task task,
		...
		try {
			...
			String errorMsg = "Error Unistalling ProductInstance "
					+ e.getMessage();
			LOGGER.info(errorMsg);

		} catch (Exception e) {
			String errorMsg = "The product "
					+ productInstance.getProductRelease().getProduct() + "-"
					+ productInstance.getProductRelease().getVersion()
		 ...
		 * updateSuccessTask(task, productInstance); LOGGER.info("Product " +
	...
	private ProductInstance getInstalledProduct(ProductRelease productRelease,
			VM vm) {
