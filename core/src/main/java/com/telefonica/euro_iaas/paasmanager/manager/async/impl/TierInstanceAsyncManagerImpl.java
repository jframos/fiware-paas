...
public class TierInstanceAsyncManagerImpl implements
		TierInstanceAsyncManager {
	
	private static Logger LOGGER =
		Logger.getLogger(TierInstanceAsyncManagerImpl.class.getName());
	...
	public void create (ClaudiaData claudiaData, TierInstance tierInstance, 
		...
		try {
			...
			LOGGER.info("The Tier Instance "
	                + tierInstance.getName()+ " has been CORRECTLY provisioned");
		} catch (InfrastructureException e)  {
			String errorMsg = " An error has ocurred in the process of creating VMs ";
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), 
					task, errorMsg, e);
		} catch (TaskNotFoundException e) {
			String errorMsg = "Unable to update task: " 
					+ e.getTask().getHref() + ". Description: " + 
					 e.getMessage();
	...
	public void update(ClaudiaData claudiaData, TierInstance tierInstance,
		...
		try {
			...
			updateSuccessTask(task, tierInstance);
			LOGGER.info("The Tier Instance "
	                + tierInstance.getName()+ " has been CORRECTLY updated");
		} catch (TaskNotFoundException e) {
			String errorMsg = "Unable to update task: " 
					+ e.getTask().getHref() + ". Description: " + 
					 e.getMessage();
	...
	public void delete(ClaudiaData claudiaData, TierInstance tierInstance, 
		...
		try{
			...
			updateSuccessTask(task, tierInstance);
			LOGGER.info("The Tier Instance "
					+ tierInstance.getName()+ " has been CORRECTLY deleted");
		} catch (InfrastructureException e)  {
			...
			String errorMsg = "Unexpected error removing VM: " 
					+ tierInstance + ". Description:" + 
					e.getMessage();
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), 
					task, errorMsg, e);
		}
     ...
     */
    private void updateErrorTaskOnInstall(TierInstance tierInstance, String vdc, 
     ...
     */
    private void updateErrorTask(TierInstance tierInstance, String vdc, Task task,
    ...
    private void updateErrorTask(Task task, String message, Throwable t) {
        ...
        LOGGER.info("An error occurs while executing an Tier Instance action. See task "
