...
public class TierInstanceAsyncManagerImpl implements TierInstanceAsyncManager {
	...
	private static Logger LOGGER = Logger
			.getLogger(TierInstanceAsyncManagerImpl.class.getName());
	...
	public void create(ClaudiaData claudiaData, TierInstance tierInstance,
		...
		try {
			...
			tierInstanceManager.create(claudiaData, tierInstance, envInstance,
					propertiesProvider);
			updateSuccessTask(task, tierInstance);
			LOGGER.info("The Tier Instance " + tierInstance.getName()
					+ " has been CORRECTLY provisioned");
		} catch (InfrastructureException e) {
			String errorMsg = " An error has ocurred in the process of creating VMs ";
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), task,
					errorMsg, e);
		} catch (TaskNotFoundException e) {
			String errorMsg = "Unable to update task: " + e.getTask().getHref()
					+ ". Description: " + e.getMessage();
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), task,
					errorMsg, e);
		} catch (Exception e) {
			String errorMsg = "Unexpected error creating productInstance. Description:"
					+ e.getMessage();
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), task,
					errorMsg, e);
		} finally {
	...
	public void update(ClaudiaData claudiaData, TierInstance tierInstance,
		...
		try {
			...
			String errorMsg = "Unexpected error updating productInstance. Description:"
					+ e.getMessage();
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), task,
					errorMsg, e);
		} finally {
	...
	public void delete(ClaudiaData claudiaData, TierInstance tierInstance,
		...
		try {
			...
			String errorMsg = "Unable to update the tier Instance. Description: "
					+ e.getMessage();
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), task,
					errorMsg, e);
		} catch (Exception e) {
			String errorMsg = "Unexpected error removing VM: " + tierInstance
					+ ". Description:" + e.getMessage();
			updateErrorTaskOnInstall(tierInstance, claudiaData.getVdc(), task,
					errorMsg, e);
		} finally {
	 ...
	 */
	private void updateSuccessTask(Task task, TierInstance tierInstance)
			throws TaskNotFoundException {
		Task loadedTask;
		try {
	 ...
	 */
	private void updateErrorTaskOnInstall(TierInstance tierInstance,
			String vdc, Task task, String errorMsg, Throwable e) {
	 ...
	 */
	private void updateErrorTask(TierInstance tierInstance, String vdc,
			Task task, String message, Throwable t) {

		String aiResource = MessageFormat.format(propertiesProvider
				.getProperty(ENVIRONMENT_BASE_URL), tierInstance.getName());
	...
	private void updateErrorTask(Task task, String message, Throwable t) {
		...
		LOGGER
				.info("An error occurs while executing an Tier Instance action. See task "
						+ task.getHref() + " for more information " + message);
	}
