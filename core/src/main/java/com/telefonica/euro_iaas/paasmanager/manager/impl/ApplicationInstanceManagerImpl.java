...
public class ApplicationInstanceManagerImpl implements
	...
	/**
	 * Install the applicationIntance on a EnvironmetInstance Precondition: 1)
	 * The EnvironmentInstance is already installed 2) The ApplicationRelease is
	 * described in the request
	 * 
	 * @param vdc
	 *            the vdc where the instance will be installed
	 * @param environmentInstance
	 *            the environmentInstance on which the application is going to
	 *            be installed
	 * @param application
	 ...
	 *             if ApplicationType provided in applicatonRelease object is
	 *             not in the paas-manager database
	 * @throws InvalidEntityException
	 *             if data in applicationRelease is not valid (artifacts, and
	 *             applciatonRelease objects)
	 * @throws AlreadyExistsEntityException
	 *             if artifcat/applicationRelease are already in the
	 *             paas-manager database
	...
	public ApplicationInstance install(String org, String vdc,
		...
		for (int i = 0; i < applicationRelease.getArtifacts().size(); i++) {
			...
			ProductInstance productInstance = getProductInstanceFromEnvironment(
					applicationRelease.getArtifacts().get(i),
					environmentInstance);
			productInstallator.installArtifact(productInstance,
					applicationRelease.getArtifacts().get(i));
		...
		ApplicationInstance applicationInstance = new ApplicationInstance(
				applicationRelease, environmentInstance);
		...
		try {
			...
			throw new InvalidEntityException(
					"Error to insert the application instante in the database: "
							+ e3.getMessage());
	...
	public void uninstall(String org, String vdc,
			EnvironmentInstance environmentInstance,
			ApplicationInstance applicationInstance)
			throws ProductInstallatorException {
		// Install The applicationRelease=n-Artifacts
		for (int i = 0; i < applicationInstance.getApplicationRelease()
				.getArtifacts().size(); i++) {
			// Install the artifact in the product instance associated
			// Obtain the VMs from EnvInstance where productRelease is installed
			ProductInstance productInstance = getProductInstanceFromEnvironment(
					applicationInstance.getApplicationRelease().getArtifacts()
							.get(i), environmentInstance);
			productInstallator.uninstallArtifact(productInstance,
					applicationInstance.getApplicationRelease().getArtifacts()
							.get(i));
	...
	private ApplicationInstance insertApplicationInstanceDB(
		...
		try {
			applicationInstance = applicationInstanceDao.load(application
					.getName());
		} catch (EntityNotFoundException e) {
			artifactsIn = application.getApplicationRelease().getArtifacts();

			for (int i = 0; i < artifactsIn.size(); i++) {
				ProductRelease productRelease = artifactsIn.get(i)
						.getProductRelease();
				try {
				...
				try {
					...
					try {
						artifact = artifactDao.create(new Artifact(artifactsIn
								.get(i).getName(),
								artifactsIn.get(i).getPath(), null,
								productRelease));
				...
				if (!isPRInAllProductReleases(productRelease,
						allProductReleases)) {
			...
			try {
				applicationRelease = applicationReleaseDao
						.create(new ApplicationRelease(application
								.getApplicationRelease().getName(), application
								.getApplicationRelease().getVersion(),
								application.getApplicationRelease()
										.getDescription(), null,

								application.getApplicationRelease()
										.getTransitableReleases(), artifactsDB));
			} catch (InvalidEntityException e3) {
				throw new InvalidEntityException(e3);
			} catch (AlreadyExistsEntityException e3) {
				throw new AlreadyExistsEntityException(e3);
			...
			try {
				applicationInstance = applicationInstanceDao
						.create(new ApplicationInstance(applicationRelease,
								application.getEnvironmentInstance(),
								application.getVdc(), application.getStatus()));
			} catch (InvalidEntityException e3) {
				throw new InvalidEntityException(ApplicationInstance.class, e3);
			} catch (AlreadyExistsEntityException e3) {
				throw new AlreadyExistsEntityException(
						ApplicationInstance.class, e3);
			}
		}
		return applicationInstance;
	}
	 ...
	 */
	private ProductInstance getProductInstanceFromEnvironment(
			Artifact artifact, EnvironmentInstance envInstance) {
		...
		for (TierInstance tierInstance : tierInstances) {
			...
			for (ProductInstance deployedProductInstance : tierInstance
					.getProductInstances()) {
	...
	private boolean isPRInAllProductReleases(ProductRelease productRelease,
		...
		for (int i = 0; i < allProductReleases.size(); i++) {
			if (productRelease.getProduct().equals(
					allProductReleases.get(i).getProduct())) {
				return true;
			}
		}
		return false;
	...
	private boolean canInstall(EnvironmentInstance envInstance,
		...
		for (int i = 0; i < envInstance.getTierInstances().size(); i++) {
			...
			for (int j = 0; j < tInstance.getProductInstances().size(); j++) {
				ProductInstance pInstance = tInstance.getProductInstances()
						.get(j);
