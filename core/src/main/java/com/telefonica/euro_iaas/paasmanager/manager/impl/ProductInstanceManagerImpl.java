...
public class ProductInstanceManagerImpl implements ProductInstanceManager {
	...
	.getLogger(ProductInstanceManagerImpl.class);

	public ProductInstance install(TierInstance tierInstance, ClaudiaData claudiaData, String envName,
			ProductRelease productRelease, List<Attribute> attributes)
			throws ProductInstallatorException,
			InvalidProductInstanceRequestException, NotUniqueResultException,
			InvalidEntityException {
		log.debug("Installing software " + productRelease.getProduct() + " in tier Instance "+ 
				tierInstance.getName() + " in vdc " + claudiaData.getVdc());

		ProductInstance productInstance = productInstallator.install(claudiaData, envName,
				tierInstance, productRelease, attributes);
	...
	public ProductInstance create(ProductInstance productInstance,
		...
		if (productRelease.getId() == null)
			try {
				...
				throw new InvalidEntityException(
						"Error to load the product release for persist the product Instance "
		...
		if (productInstance.getId() == null)
			try {
				...
				// TODO Auto-generated catch block
				throw new InvalidEntityException(
						"Error to persist the product Instance "
								+ productInstance.getName() + " : "
								+ e.getMessage());
			} catch (AlreadyExistsEntityException e) {
				// TODO Auto-generated catch block
	...
	public ProductInstance create(ProductInstance productInstance)
		...
		try {
			...
			productRelease = productReleaseManager.load(productInstance
					.getProductRelease().getProduct()
					+ "-" + productInstance.getProductRelease().getVersion());
			productInstance.setProductRelease(productRelease);
		} catch (EntityNotFoundException e) {
			String errorMessage = "The Product Release Object "
					+ productRelease.getId() + " is " + "NOT valid";
		...
		try {
			...
			try {
				...
				String errorMessage = "The Product Instance Object "
						+ productInstance.getName() + " is " + "NOT valid";
				throw new InvalidProductInstanceRequestException(errorMessage);
	...
	public void configure(ClaudiaData claudiaData, ProductInstance productInstance,
		...
		log.debug("Configuring software " + productInstance.getProductRelease().getProduct() + " " + productInstance.getName());
