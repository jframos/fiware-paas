 ...
 */
public class ApplicationInstanceAsyncManagerImpl implements
		ApplicationInstanceAsyncManager {

	private static Logger LOGGER = Logger
			.getLogger(ProductInstanceAsyncManagerImpl.class.getName());
	...
	public void install(String org, String vdc, String environmentInstanceName,
		...
		try {
			EnvironmentInstance environmentInstance = environmentInstanceDao
					.load(environmentInstanceName);

			ApplicationInstance applicationInstance = applicationInstanceManager
					.install(org, vdc, environmentInstance, applicationRelease);
			...
			LOGGER.info("Application " + applicationRelease.getName() + '-'
					...
					+ " installed successfully " + " on Environment "
			...
			updateErrorTask(prNFE.getProductRelease().getName(),
					PRODUCT_RELEASE_BASE_URL, task, errorMsg, prNFE);
		} catch (ApplicationTypeNotFoundException atNFE) {
			String errorMsg = atNFE.getMessage();
			updateErrorTask(atNFE.getApplicationType().getName(),
					APPLICATION_TYPE_BASE_URL, task, errorMsg, atNFE);
		} catch (InvalidEntityException iee) {
			String errorMsg = iee.getMessage();
			updateErrorTask(applicationRelease.getName(),
					APPLICATION_RELEASE_BASE_URL, task, errorMsg, iee);
		} catch (AlreadyExistsEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (ProductInstallatorException e) {
			String errorMsg = "Error installing an application. Description:"
					+ e.getMessage();
			updateErrorTask("Error installing an application",
	...
	public void uninstall(String org, String vdc,
			String environmentInstanceName, String applicationName, Task task,
			String callback) {

		ApplicationInstance applicationInstance = null;
		try {
			applicationInstance = applicationInstanceManager.load(vdc,
					applicationName);
			EnvironmentInstance environmentInstance = environmentInstanceDao
					.load(environmentInstanceName);
			applicationInstanceManager.uninstall(org, vdc, environmentInstance,
					applicationInstance);
			updateSuccessTask(task, environmentInstance);
			LOGGER.info("Application " + applicationInstance.getName() + '-'
					+ " uninstalled successfully " + " on Environment "
					+ environmentInstanceName);
		} catch (EntityNotFoundException e) {
			String errorMsg = e.getMessage();
			updateErrorTask(environmentInstanceName,
	...
	private void updateSuccessTask(Task task,
		...
		String piResource = MessageFormat.format(propertiesProvider
				.getProperty(ENVIRONMENT_INSTANCE_BASE_URL),
	...
	/**
	 * Update the task with necessary information when the task is wrong because
	 * the environmentInstance does not exist in the system.
	 */
	private void updateErrorTask(String resourceName, String baseUrl,
			Task task, String message, Throwable t) {
		String piResource = MessageFormat.format(propertiesProvider
				.getProperty(baseUrl), resourceName); // the name
	...
	private void updateErrorTask(Task task, String message, Throwable t) {
		...
		LOGGER
				.info("An error occurs while installing an application release on a"
						+ " environment. See task "
						+ task.getHref()
