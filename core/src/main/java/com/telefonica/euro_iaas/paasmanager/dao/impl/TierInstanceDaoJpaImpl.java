...
public class TierInstanceDaoJpaImpl extends
		AbstractBaseDao<TierInstance, String> implements TierInstanceDao {
	...
	public List<TierInstance> findByCriteria(TierInstanceSearchCriteria criteria)
		...
		List<TierInstance> tierInstances = setOptionalPagination(criteria,
				baseCriteria).list();

		if (criteria.getProductInstance() != null) {
			tierInstances = filterByProductInstance(tierInstances, criteria
					.getProductInstance());
		}

		if (criteria.getService() != null) {
			tierInstances = filterByService(tierInstances, criteria
					.getService());
		}
		if (criteria.getEnvironmentInstance() != null
				&& criteria.getVdc() != null) {
			tierInstances = filterByEnvironmentInstanceVDC(tierInstances,
					criteria.getEnvironmentInstance(), criteria.getVdc());
		}

		return tierInstances;
	}

	private List<TierInstance> filterByEnvironmentInstanceVDC(
	...
	private List<TierInstance> filterByService(
		...
		List<TierInstance> result = new ArrayList<TierInstance>();
		/*
		 * for (TierInstance tierInstance : tierInstances) { List<Service>
		 * services = tierInstance.getServices();
		 * 
		 * for (Service serv : services) { if
		 * (serv.getName().equals(serviceInput.getName()))
		 * result.add(tierInstance); } }
		 */
		return result;
	}
	 ...
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.dao.TierInstanceDao#findByTierInstanceId
	 * (java.lang.Long)
	 */

	public TierInstance findByTierInstanceId(Long tierInstanceId)
			throws EntityNotFoundException {
		Query query = entityManager
				.createQuery("select p from TierInstance p join "
		...
		try {
			...
			String message = " No TierInstance found in the database with name: "
					+ Long.toString(tierInstanceId);
			throw new EntityNotFoundException(null, e.getMessage(), message);
		}
		return tierInstance;
	}

	public TierInstance findByTierInstanceName(String tierInstanceName)
			throws EntityNotFoundException {
		Query query = entityManager
				.createQuery("select p from TierInstance p join "
		...
		try {
			...
			throw new EntityNotFoundException(TierInstance.class, "name",
					tierInstanceName);
