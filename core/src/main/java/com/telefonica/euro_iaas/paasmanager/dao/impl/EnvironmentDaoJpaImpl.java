...
public class EnvironmentDaoJpaImpl extends AbstractBaseDao<Environment, String>
		implements EnvironmentDao {

	private static final String QUERY_LOAD_BY_TWO_FIELDS 
		= "SELECT o FROM {0} o WHERE o.{1} = :{1} and o.{2} =:{2}";
	...
	/*public Environment load(String envName) throws EntityNotFoundException {
		...
		criteria.setEnvironmentName(envName);
		
		List<Environment> environments = findByCriteria(criteria);
	...
	/*public Environment load(String envName, String vdc) throws EntityNotFoundException {
		...
		try {
			environment = findByEnvironmentNameVdc(envName, vdc);
		} catch (Exception e) {
			environment =  findByEnvironmentNameVdcNoTiers (envName, vdc);
		}
		environments.add(environment);
		return filterEqualTiers(environments).get(0);
	}*/
	...
	/*public Environment load(String envName, String vdc) throws EntityNotFoundException {	
        ...
        String query = MessageFormat.format(QUERY_LOAD_BY_TWO_FIELDS, 
        		"Environment", "name", "vdc");           
        try {
            Environment environment = (Environment) getEntityManager().createQuery(query)
            		.setParameter("vdc", vdc)
            		.setParameter("name", envName)
            		.getSingleResult();
            return environment;
	...
	public List<Environment> findByCriteria(EnvironmentSearchCriteria criteria) {
		...
		if (criteria.getOrg() != null && criteria.getVdc() != null && criteria.getEnvironmentName () !=null) {
			environments = filterByEnvironment(environments, criteria.getOrg(),
					criteria.getVdc(), criteria.getEnvironmentName());
			return environments;
		}

		if (criteria.getOrg() != null && criteria.getVdc() != null) {
			environments = filterByOrgAndVdc(environments, criteria.getOrg(),
					criteria.getVdc());
			return environments;
		}
	...
	/*public List<Environment> findByCriteria(
		...
		if (!StringUtils.isEmpty(criteria.getOrg())) {
			baseCriteria.add(Restrictions.eq(Environment.ORG_FIELD,
					criteria.getOrg()));
		}
		if (!StringUtils.isEmpty(criteria.getVdc())) {
			baseCriteria.add(Restrictions.eq(Environment.VDC_FIELD,
					criteria.getVdc()));
		}
		if (!StringUtils.isEmpty(criteria.getEnvironmentName())) {
			baseCriteria.add(Restrictions.eq(Environment.ENVIRONMENT_NAME_FIELD,
		...
		return filterEqualTiers(environments);
	}*/

	private List<Environment> filterByOrgAndVdc(List<Environment> environments,
			String org, String vdc) {
		List<Environment> result = new ArrayList<Environment>();
		for (Environment environment : environments) {
			if (environment.getVdc() == null)
				continue;
			if (environment.getOrg().equals(org)
					&& environment.getVdc().equals(vdc)) {
	...
	private List<Environment> filterByOrg(List<Environment> environments,
			String org) {
		List<Environment> result = new ArrayList<Environment>();
		for (Environment environment : environments) {
			if (environment.getOrg().equals(org)
					&& environment.getVdc() == null) {
				result.add(environment);
			}
		}
		return result;
	...
	private List<Environment> filterByEnvironment(List<Environment> environments, String org, String vdc,
		...
		for (Environment environment : environments) {
			if (environment.getOrg().equals(org)
					&& environment.getVdc() != null && environment.getVdc().equals(vdc) &&
	 ...
	 * 
	 * @see
	 * com.telefonica.euro_iaas.paasmanager.dao.TierDao#findByTierId(java.lang
	 * .String)
	 */
	private Environment findByEnvironmentName(String envName)
			throws EntityNotFoundException {
		Query query = entityManager
				.createQuery("select p from Environment p join "
		...
		try {
			...
			String message = " No Environment found in the database with name: "
					+ envName;
			throw new EntityNotFoundException(Environment.class, "name",
					envName);
		}
		return environment;
	}
	
	private Environment findByEnvironmentNameNoTiers(String envName)
			throws EntityNotFoundException {
		Query query = entityManager
				.createQuery("select p from Environment p where p.name = :name");
		query.setParameter("name", envName);
		Environment environment = null;
		try {
			environment = (Environment) query.getSingleResult();
	...
	private Environment findByEnvironmentNameVdc(String envName, String vdc)
	throws EntityNotFoundException {
		Query query = entityManager
				.createQuery("select p from Environment p join "
	...
	private Environment findByEnvironmentNameVdcNoTiers(String envName, String vdc)
		throws EntityNotFoundException {
		Query query = entityManager
				.createQuery("select p from Environment p where p.name = :name and p.vdc = :vdc");
		query.setParameter("name", envName);
		query.setParameter("vdc", vdc);
		Environment environment = null;
		try {
	 ...
	 * @return environments
	 */
	private List<Environment> filterByTier(List<Environment> environments,
			Tier tierInput) {
		List<Environment> result = new ArrayList<Environment>();
		for (Environment environment : environments) {
