...
import com.telefonica.euro_iaas.paasmanager.model.dto.VMDto;
...
public class EnvironmentInstance extends InstallableInstance {
	...
	public EnvironmentInstanceDto toDto() {
		...
		/*
		 * if (envInstance.getEnvironment() != null) { EnvironmentDto envDto =
		 * convertToDto(envInstance .getEnvironment());
		...
		if (this.getEnvironment() != null) {
			if (this.getEnvironment().getTiers() != null) {
				for (Tier tier : getEnvironment().getTiers()) {
					TierPDto tierPDto = createTierPDto(tier, this
							.getTierInstances());
	...
	private TierPDto createTierPDto(Tier tier, List<TierInstance> lTierInstance) {
		...
		TierPDto tierPDto = new TierPDto(tier.getName(), tier
				.getMaximumNumberInstances(), tier.getMinimumNumberInstances(),
				tier.getInitialNumberInstances(), productReleasedto, tier
						.getFlavour(), tier.getImage(), tier.getIcono(),
				securitygroup, tier.getKeypair(), tier.getFloatingip());
	...
	private TierInstancePDto createInstancePDto(TierInstance tierInstance) {
		...
		{
		  tierInstancePDto = new TierInstancePDto(tierInstance
					.getName(), productInstanceDtos, null, tierInstance.getStatus(), tierInstance.getTaskId());
		}
		else
		{
		 tierInstancePDto = new TierInstancePDto(tierInstance
				.getName(), productInstanceDtos, tierInstance.getVM().toDto(), tierInstance.getStatus(), tierInstance.getTaskId());
		}
