...
public class EnvironmentInstanceTest extends TestCase {
	...
	public void setUp() throws Exception {
		...
		TierInstance tierInstance = new TierInstance(tier, "ovf",
				"tierInstance", vm);
	...
	public void testCreateEnvironment() throws Exception {
		...
		assertEquals(envIns.getEnvironment().getTiers().get(0).getName(),
				"tierName");
		assertEquals(envIns.getTierInstances().size(), 1);
		assertEquals(envIns.getTierInstances().get(0).getProductInstances()
				.size(), 1);
		assertEquals(envIns.getTierInstances().get(0).getProductInstances()
				.get(0).getName(), "produInst");
		assertEquals(envIns.getTierInstances().get(0).getProductInstances()
				.get(0).getProductRelease().getProduct(), "product");
	...
	public void testCreateEnvironmentToPDto() throws Exception {
		...
		assertEquals(enviromentIsntanceDto.getTiers().size(), envResult
				.getTiers().size());
		assertEquals(enviromentIsntanceDto.getTiers().get(0).getProductReleaseDtos()
				.size(), 1);
		assertEquals(enviromentIsntanceDto.getTiers().get(0).getTierInstances()
				.size(), 1);
		assertEquals(enviromentIsntanceDto.getTiers().get(0).getTierInstances()
				.get(0).getTierInstanceName(), "tierInstance");
		assertEquals(enviromentIsntanceDto.getTiers().get(0).getTierInstances()
				.get(0).getVM().getFqn(), "fqn");
